...
{
    public partial class VehicleCheckList : Form
    {
        ...
        private string _licensePlateFilter = "";      // Cột hiện đang sắp xếp
        ...
        private async Task LoadVehicle()
        {
            try
            {
                var vehicles = await _ticketIssuanceService.GetVehiclesCheckAsync();
        ...
        private async void VehicleCheckList_Load(object sender, EventArgs e)
        {
            await LoadVehicle();
        }
        ...
        private async void gridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (response != null)
            {
                OpenChildForm(new Check(response));
            }
            else
            {
